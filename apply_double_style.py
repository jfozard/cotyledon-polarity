...
def test_unet(model_fpath, im, output_fpath=None, ts=TS, rotate=0):
    ...
    return as_image[np.newaxis,:,:]/np.max(as_image)
...
def main(model_fpath, input_fpath, output_fpath, rotate):
    ...
    test_unet(model_fpath, input_image, output_fpath, rotate=int(rotate))
